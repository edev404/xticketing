...
import { ClearingDataPassages, CollectsClearingPassages, DetailsClearingPassages, PresettlementFilter } from '../../models/models';
...
export class CreatePassageComponent implements OnInit {
  ...
  baseList: any[] = [
    { id: 2, name: 'Pasajes', code: 'passage' },
  ];
  cities!: any[];
  collectors!: any[];
  ...
  onSearchCity(event) {
    ...
    if (!event) return;
    if (!this.listOfDataFilter) this.listOfDataFilter = this.cities;

    if (event || (event && event.trim() != '')) {
      data = this.listOfDataFilter.filter((current) => {
        return current.name.toUpperCase().includes(event.toUpperCase());
      });
      if (data) this.cities = data;
    } else {
      if (this.listOfDataFilter) this.cities = this.listOfDataFilter;
    }    
  }
