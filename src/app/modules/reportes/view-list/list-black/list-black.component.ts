...
@Component({
  selector: 'app-list-black',
  templateUrl: './list-black.component.html',
  styleUrls: ['./list-black.component.scss']
})
export class ListBlackComponent implements OnInit {
  ...
  async loadData() {
    const resp = await this.api.getWithList(this.utils.getBasicEndPoint('blacklist/refresh'));
    if (resp.status === this.utils.successMessage) {
      this.listOfData = resp.data.list;
    } else {                         
